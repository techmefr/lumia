...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "ai_provider", sa.Enum("MISTRAL", "OPENAI", "CUSTOM", name="aiprovider"), nullable=True
        ),
...
def downgrade() -> None:
    ...
    # Dropping a table does not drop the enum type it used, and alembic does not generate this.
    # Left behind, they made the next `upgrade head` fail on `type "lang" already exists`, so a
    # rollback was one-way: you could go back but never forward again.
    for enum_name in ("vote", "sourcetype", "orbitposition", "theme", "aiprovider", "role", "lang"):
