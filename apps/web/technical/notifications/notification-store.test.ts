...
/** Instants are written in UTC and read back through an explicit timezone, never the host clock. */
function instant(utcIso: string): Temporal.Instant {
	return Temporal.Instant.from(utcIso);
}

const AWAKE = instant('2026-09-09T14:00:00Z');
...
describe('isQuietHour', () => {
	it('is never quiet when both bounds are the same hour', () => {
		expect(isQuietHour(instant('2026-09-09T03:00:00Z'), 8, 8, 'UTC')).toBe(false);
	});

	describe('a window inside one day', () => {
		it('is quiet on the opening hour', () => {
			expect(isQuietHour(instant('2026-09-09T09:00:00Z'), 9, 17, 'UTC')).toBe(true);
		});

		it('is quiet in the middle', () => {
			expect(isQuietHour(instant('2026-09-09T13:00:00Z'), 9, 17, 'UTC')).toBe(true);
		});

		it('is awake again on the closing hour, which is exclusive', () => {
			expect(isQuietHour(instant('2026-09-09T17:00:00Z'), 9, 17, 'UTC')).toBe(false);
		});

		it('is awake before it opens', () => {
			expect(isQuietHour(instant('2026-09-09T08:59:00Z'), 9, 17, 'UTC')).toBe(false);
		});
	});

	describe('a window wrapping past midnight', () => {
		it('is quiet on the opening hour, before midnight', () => {
			expect(isQuietHour(instant('2026-09-09T22:00:00Z'), 22, 8, 'UTC')).toBe(true);
		});

		it('is quiet after midnight', () => {
			expect(isQuietHour(instant('2026-09-09T03:00:00Z'), 22, 8, 'UTC')).toBe(true);
		});

		it('is awake on the closing hour', () => {
			expect(isQuietHour(instant('2026-09-09T08:00:00Z'), 22, 8, 'UTC')).toBe(false);
		});

		it('is awake in the middle of the afternoon', () => {
			expect(isQuietHour(instant('2026-09-09T15:00:00Z'), 22, 8, 'UTC')).toBe(false);
		});
	});

	// The whole point of an explicit timezone parameter: the same instant on the wire must read as
	// quiet in one reader's zone and awake in another's, regardless of where the code runs.
	describe('the same instant in two opposite timezones', () => {
		it('is quiet in Tokyo, at 23:00 local, but awake in Paris at the same instant', () => {
			// 14:00 UTC = 23:00 in Tokyo (UTC+9) = 16:00 in Paris (UTC+2, summer).
			const noon = instant('2026-09-09T14:00:00Z');
			expect(isQuietHour(noon, 22, 8, TOKYO)).toBe(true);
			expect(isQuietHour(noon, 22, 8, PARIS)).toBe(false);
		});
...
describe('notificationFor', () => {
	...
	it('says nothing on the very first poll, so enabling never announces the backlog', () => {
		expect(notificationFor(120, null, settings(), AWAKE, 'UTC')).toBeNull();
	});

	it('says nothing when the unread total has not moved', () => {
		expect(notificationFor(40, 40, settings(), AWAKE, 'UTC')).toBeNull();
	});

	it('says nothing when the total went down because articles were read elsewhere', () => {
		expect(notificationFor(12, 40, settings(), AWAKE, 'UTC')).toBeNull();
	});

	it('stays silent below the threshold', () => {
		expect(notificationFor(42, 40, settings({ threshold: 3 }), AWAKE, 'UTC')).toBeNull();
	});

	it('speaks exactly on the threshold', () => {
		expect(notificationFor(43, 40, settings({ threshold: 3 }), AWAKE, 'UTC')).not.toBeNull();
	});

	it('counts the new articles rather than the total', () => {
		expect(notificationFor(140, 100, settings({ threshold: 3 }), AWAKE, 'UTC')).toContain('40');
	});

	it('uses the singular wording when a threshold of one lets a lone article through', () => {
		const message = notificationFor(41, 40, settings({ threshold: 1 }), AWAKE, 'UTC');
		expect(message).toBe('1 new article to read');
	});

	it('uses the plural wording beyond one', () => {
		expect(notificationFor(45, 40, settings({ threshold: 1 }), AWAKE, 'UTC')).toBe(
			'5 new articles to read'
		);
	});

	it('stays silent during quiet hours even with plenty of new articles', () => {
		const night = instant('2026-09-09T02:00:00Z');
		expect(
			notificationFor(200, 40, settings({ quietFromHour: 22, quietToHour: 8 }), night, 'UTC')
		).toBeNull();
	});

	it('speaks at the same volume once the quiet window closes', () => {
		const morning = instant('2026-09-09T08:00:00Z');
		expect(
			notificationFor(200, 40, settings({ quietFromHour: 22, quietToHour: 8 }), morning, 'UTC')
		).not.toBeNull();
	});

	// The quiet window is evaluated in the reader's own zone: the same instant is quiet hours in
	// Tokyo but the middle of the afternoon in Paris.
	it('evaluates the quiet window in the reader timezone, not a fixed one', () => {
		const sameInstant = instant('2026-09-09T14:00:00Z');
		const config = settings({ quietFromHour: 22, quietToHour: 8, threshold: 1 });

		expect(notificationFor(45, 40, config, sameInstant, TOKYO)).toBeNull();
		expect(notificationFor(45, 40, config, sameInstant, PARIS)).not.toBeNull();
	});

	it('follows the interface language, so a message never lags a language change', () => {
		setLocale('fr');
		const french = notificationFor(45, 40, settings({ threshold: 1 }), AWAKE, 'UTC');
		setLocale('de');
		const german = notificationFor(45, 40, settings({ threshold: 1 }), AWAKE, 'UTC');
		setLocale('en');
		const english = notificationFor(45, 40, settings({ threshold: 1 }), AWAKE, 'UTC');
		expect(new Set([french, german, english]).size).toBe(3);
	});
});
